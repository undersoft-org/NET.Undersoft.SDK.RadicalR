...
namespace RadicalR
{
    public class CommandValidator<TDto> : CommandValidatorBase<Command<TDto>> where TDto : Dto
    {
        ...
        protected void ValidateLength(int min, int max, params Expression<Func<Command<TDto>, string>>[] members)
        {
            foreach (var member in members)
            {
                RuleFor(member)
        ...
        protected void ValidateCount(int min, int max, params Expression<Func<Command<TDto>, ICollection>>[] members)
        {
            foreach (var member in members)
            {
                RuleFor(member)
        ...
        protected void ValidateNotEqual(object item, params Expression<Func<Command<TDto>, object>>[] members)
        {
            foreach (var member in members)
            {
                RuleFor(member)
        ...
        protected void ValidateEqual(object item, params Expression<Func<Command<TDto>, object>>[] members)
        {
            foreach (var member in members)
            {
                RuleFor(member)
